...
/**compute taxes using if statements tto check filing status and compute the tax based on status   */
public class ComputeTaxWithMethod {
	...
	public static double computeTax(double income, int r1, int r2, int r3,
														int r4, int r5 ) 
	{
		double tax=0;
		...
		else if(income<=r4) tax= r1*0.10+(r2-r1)*0.15+ (r2-r1)*0.15+
				(r3-r2)*0.27+ (income-r3)*0.30;
		else if(income<=r5) tax= r1*0.10+(r2-r1)*0.15+(r4-r3)*0.30+
				(r3-r2)*0.27+ (income-r4)*0.35;
		else tax= r1*0.10+ (r2-r1)*0.15+ (r3-r2)*0.27+ (r4-r3)*0.30+
				(r5-r4)*0.35+ (income-r5)*0.386;
		return tax;			
	}
	public static double  computeTax(int status, double income) {
		...
		case 1: return
				computeTax(income, 12000, 46700, 112850, 171950, 3077050);
		case 2: return
